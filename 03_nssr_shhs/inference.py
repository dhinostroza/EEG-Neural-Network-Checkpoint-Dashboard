...
def convert_edf_to_parquet(edf_path, output_path):
    ...
    try:
        ...
        # Use MNE's built-in type detection (handles BDF 'A1', 'A2'... correctly)
        if 'eeg' in raw:
            raw.pick_types(eeg=True, exclude='bads')
            # If multiple channels, just take the first one for consistency with current pipeline
            # (The existing model is single-channel)
            raw.pick_channels([raw.ch_names[0]])
            print(f"Selected channel: {raw.ch_names[0]}")
        
        elif 'EEG' in raw.ch_names:
            raw.pick_channels(['EEG'])
        elif 'EEG(sec)' in raw.ch_names:
             raw.pick_channels(['EEG(sec)'])
        else:
            # Fallback for some EDFs where type might not be set or names are weird
            available = raw.ch_names
            print(f"Warning: 'EEG' channel not found in {available}. Using first channel: {available[0]}")
            raw.pick_channels([available[0]])
            
        # Debug info
        print(f"Data info: {raw.info}")
        print(f"Duration: {raw.times[-1]}s, SFreq: {raw.info['sfreq']}")
            
        # 3. Resample to 64Hz (from script)
        raw.resample(64, npad="auto")
        
        # 3.5. Scale BDF data (Volts -> uV) 
        # BioSemi natively stores in uV-like integers, but MNE converts to Volts.
        # Our spectrogram/normalization might expect larger values.
        if ext == '.bdf':
            print("Scaling BDF data by 1e6 (V -> uV)...")
